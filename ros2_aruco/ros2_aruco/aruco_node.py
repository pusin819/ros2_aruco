...
class ArucoNode(rclpy.node.Node):
    def __init__(self):
        ...
        self.img_id_1 = (
            self.get_parameter("img_id_1").get_parameter_value().integer_value
        )
        self.get_logger().info(f"img_id_1: {self.img_id_1}")
        self.img_id_2 = (
            self.get_parameter("img_id_2").get_parameter_value().integer_value
        )
        self.get_logger().info(f"img_id_2: {self.img_id_2}")
    ...
    def image_callback(self, img_msg):
        ...
        if marker_ids is not None:
            ...
            if len(markers.marker_ids) > 0:
                ...
                for pose, id in zip(pose_array.poses, markers.marker_ids):
                    if(id == self.img_id_1):
                        self.get_logger().info("id 1")
                        data_0_id = 0x02
                    elif(id == self.img_id_2):
                        self.get_logger().info("id 2")
                        data_0_id = 0x03
                    else:
                        # self.get_logger().info("id not found")
                        return;
                    ...
                    self.get_logger().info(f"Position: {position_x}")
                    
                    data_array1 = np.append(data_array1, data_0_id)
